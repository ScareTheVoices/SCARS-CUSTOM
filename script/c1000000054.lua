...
function s.initial_effect(c)
	...
	--Special Summon from hand by destroying required card (Hard Once Per Turn)
	...
	--Instant Floating / On-Destruction Loop
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(id,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F) -- Mandatory trigger
	e2:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_DAMAGE_STEP)
	e2:SetCode(EVENT_DESTROYED)
	e2:SetCondition(s.instantcon)
	e2:SetTarget(s.instanttg)
	e2:SetOperation(s.instantop)
	c:RegisterEffect(e2)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- Applies the opponent counter-lock block system before execution
	local e1=Effect.CreateEffect(c)
...
-- Instant Revival Filter Parameters
function s.instantcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_MZONE)
end
function s.rvspfilter(c,e,tp)
	return c:IsCode(1000000000) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.instanttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
function s.instantop(e,tp,eg,ep,ev,re,r,rp)
...
-- "Control Only 1" Rule Filters (Dreadnought Logic Alignment)
...
-- Banish Field Wipe Logic
function s.excludefilter(c,sc)
	return c~=sc and c:IsAbleToBanish()
end
...
function s.banishop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		if Duel.Banish(g,POS_FACEUP,REASON_EFFECT)>0 then
...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsDisabled() then
		...
		-- If successfully negated, recycle 1 of your banished monsters to your deck
...
function s.apply_granted_effect(c)
	...
	-- On-Summon Gained Effect: Mass Banish Field Wipe + Burn Math
